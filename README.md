...
- The browser chrome gets `--chrome`, a shade off `--bg`, so the bars frame the page instead
  of pretending to be it. It takes two mechanisms: `theme-color` for Chrome/Android and iOS
  Safari before 26, and the `background-color` on `#chrome` for iOS 26+, which ignores
  `theme-color` and instead samples fixed elements near the viewport edges. `#chrome` is an
  empty `opacity: 0` div that exists only to be sampled - Safari parses it anyway, though
  `display: none` would drop it - and it must stay the only fixed element declaring a colour,
  or the plate and the grain compete for the sample. Keep the three values in sync.
